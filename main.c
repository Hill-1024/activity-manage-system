...
void check_expiry(Activity *act) {
    ...
    if (strcmp(now, act->deadline) > 0) {
        ...
        if(strcmp(act->status, "已结束") == 0) strcpy(act->status, "报名中");
    }
}
...
int get_json_int(const char *json, const char *key) {
    char search[64]; const char *p; sprintf(search, "\"%s\":", key);
    p = strstr(json, search); if (!p) return -1; return atoi(p + strlen(search));
}
void get_json_str(const char *json, const char *key, char *dest) {
    char search[64]; const char *p; int i = 0; sprintf(search, "\"%s\":\"", key);
    p = strstr(json, search); if (!p) { dest[0] = '\0'; return; }
    p += strlen(search); while(*p != '\"' && *p != '\0') dest[i++] = *p++; dest[i] = '\0';
}
...
void load_students() {
    ...
    while (fscanf(fp, "%s %s %s %s %s", id, n, p, ph, c) != EOF) {
        ...
        strcpy(node->id,id);strcpy(node->name,n);strcpy(node->password,p);strcpy(node->phone,ph);strcpy(node->class_name,c);
        node->next=student_head; student_head=node;
    }
    fclose(fp);
}
void save_activities() {
    ...
    while(curr) {
        ...
        fprintf(fp, "%d %s %s %s %d %s %s ", curr->id, curr->name, curr->category, curr->location, curr->max_capacity, curr->status, curr->deadline);
        if (!curr->enrolled_head) fprintf(fp, "None");
        else {
            EnrolledNode *e = curr->enrolled_head;
            while(e) { fprintf(fp, "%s", e->student_id); if (e->next) fprintf(fp, "|"); e = e->next; }
        }
        fprintf(fp, "\n"); curr = curr->next;
    }
    fclose(fp);
...
void load_activities() {
    ...
    while(fscanf(fp, "%d %s %s %s %d %s %s %s", &id, name, cat, loc, &max, stat, ddl, list) != EOF) {
        ...
        node->id = id; if(id>global_id_counter) global_id_counter=id;
        strcpy(node->name, name); strcpy(node->category, cat); strcpy(node->location, loc);
        node->max_capacity = max; strcpy(node->status, stat); strcpy(node->deadline, ddl);
        node->enrolled_head = NULL; node->current_count = 0;
        if (strcmp(list, "None") != 0) {
            char *token = strtok(list, "|");
            while(token) {
                EnrolledNode *en = (EnrolledNode *)malloc(sizeof(EnrolledNode));
                strcpy(en->student_id, token); en->next = node->enrolled_head;
                node->enrolled_head = en; node->current_count++; token = strtok(NULL, "|");
            }
        }
        check_expiry(node); // 加载时立即检查
        node->next = activity_head; activity_head = node;
    }
    fclose(fp);
}
...
int register_student(const char *id, const char *name, const char *pwd, const char *ph, const char *cls) {
    Student *c = student_head; while(c) { if(strcmp(c->id,id)==0) return 0; c=c->next; }
    Student *n = (Student *)malloc(sizeof(Student));
    strcpy(n->id,id);strcpy(n->name,name);strcpy(n->password,pwd);strcpy(n->phone,ph);strcpy(n->class_name,cls);
    n->next=student_head; student_head=n; save_students(); return 1;
}
...
int enroll_student(int act_id, const char *stu_id) {
    ...
    while(curr) {
        if(curr->id == act_id) {
            ...
            while(chk) { if(strcmp(chk->student_id, stu_id)==0) return 0; chk=chk->next; }
            EnrolledNode *node = (EnrolledNode *)malloc(sizeof(EnrolledNode));
            strcpy(node->student_id, stu_id); node->next = curr->enrolled_head;
            curr->enrolled_head = node; curr->current_count++;
            save_activities(); return 1;
        }
        curr = curr->next;
    }
    return -404;
...
void delete_activity(const int id) {
    ...
    while(curr) {
        if(curr->id == id) {
            ...
            free(curr); save_activities(); return;
        }
...
static void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data) {
  if (ev == MG_EV_HTTP_MSG) {
    ...
    char body[2048] = {0};
    if (hm->body.len > 0 && hm->body.len < 2047) memcpy(body, hm->body.buf, hm->body.len);

    if (mg_match(hm->uri, mg_str("/api/login"), NULL)) {
        char u[50]={0}, p[50]={0}; get_json_str(body, "username", u); get_json_str(body, "password", p);
        if(strcmp(u,admin)==0 && strcmp(p,adminPassword)==0) mg_http_reply(c, 200, "", "{\"status\":\"success\",\"role\":\"admin\"}");
        else {
             Student *s=student_head; int f=0;
             while(s){if(strcmp(s->id,u)==0 && strcmp(s->password,p)==0){f=1;break;}s=s->next;}
        ...
        get_json_str(body, "id", id); get_json_str(body, "name", n); get_json_str(body, "password", p);
        get_json_str(body, "phone", ph); get_json_str(body, "class", cl);
        ...
        char *json = (char *)malloc(1024 * 64); strcpy(json, "[");
        Activity *curr = activity_head; char item[1024];
        while(curr) {
            check_expiry(curr);
            sprintf(item, "{\"id\":%d, \"name\":\"%s\", \"category\":\"%s\", \"location\":\"%s\", \"deadline\":\"%s\", \"max\":%d, \"count\":%d, \"status\":\"%s\"}",
                curr->id, curr->name, curr->category, curr->location, curr->deadline, curr->max_capacity, curr->current_count, curr->status);
            strcat(json, item); if(curr->next) strcat(json, ","); curr = curr->next;
        }
        strcat(json, "]"); mg_http_reply(c, 200, "Content-Type: application/json\r\n", "%s", json); free(json);
    }
    else if (mg_match(hm->uri, mg_str("/api/add"), NULL)) {
        char name[100]={0}, cat[50]={0}, loc[100]={0}, ddl[25]={0};
        int max = get_json_int(body, "max");
        get_json_str(body, "name", name); get_json_str(body, "category", cat);
        get_json_str(body, "location", loc); get_json_str(body, "deadline", ddl);
        Activity *node = (Activity *)malloc(sizeof(Activity));
        node->id = ++global_id_counter;
        strcpy(node->name, name); strcpy(node->category, cat);
        strcpy(node->location, loc); strcpy(node->deadline, ddl);
        node->max_capacity = max; strcpy(node->status, "报名中");
        node->current_count = 0; node->enrolled_head = NULL;
        node->next = activity_head; activity_head = node; save_activities();
        mg_http_reply(c, 200, "", "{\"status\":\"added\"}");
    }
    else if (mg_match(hm->uri, mg_str("/api/enroll"), NULL)) {
        int aid=get_json_int(body,"id"); char sid[20]={0}; get_json_str(body,"student_id",sid);
    ...
    else if (mg_match(hm->uri, mg_str("/api/delete"), NULL)) { delete_activity(get_json_int(body,"id")); mg_http_reply(c, 200, "", "{\"status\":\"deleted\"}"); }
                ...
                while(e){
                    ...
                    while(s) {
                        if(strcmp(s->id, e->student_id) == 0) {
                            ...
                            sprintf(item, "{\"id\":\"%s\", \"name\":\"%s\", \"classname\":\"%s\", \"phone\":\"%s\"}",
        ...
        free(buf);
         ...
         int a=0,b=0,cs=0,d=0; Activity *cu=activity_head;
         while(cu){if(strcmp(cu->category,"学术")==0)a++;else if(strcmp(cu->category,"文艺")==0)b++;else if(strcmp(cu->category,"体育")==0)cs++;else d++;cu=cu->next;}
         mg_http_reply(c, 200, "Content-Type: application/json\r\n", "{\"academic\":%d,\"arts\":%d,\"sports\":%d,\"other\":%d}", a,b,cs,d);
    }
    else { struct mg_http_serve_opts o={.root_dir="./frontend"}; mg_http_serve_dir(c, hm, &o); }
  }
}
